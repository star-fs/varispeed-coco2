 ...
 * 1. Motorola MC6809 8-Bit Microprocessor Programming Manual
 * 2. 6809 Instruction Set Reference and Opcode Maps (http://www.kenseglerdesigns.com/shipp/6809/6809.htm)
 * 3. 6809 Instruction Execution Cycles & Timing Specifications
 * 4. Implementation inspired by Dave Shumann's 6809 JS CPU core
 */
...
*/
  ...
  return {
    steps: function (Ts) {
      while (Ts > 0) {
        if (firq_asserted && !(CC & F_FIRQMASK)) {
          ...
          T += 10;
          Ts -= 10;
        } else if (irq_asserted && !(CC & F_IRQMASK)) {
          CC |= F_ENTIRE;
          PSHS(0xFF);
          CC |= F_IRQMASK;
          PC = ReadWord(vecIRQ);
    ...
    set_firq_line: function (state) {
      firq_asserted = state;
    },
    set_irq_line: function (state) {
      irq_asserted = state;
    },
    T: function () {
      return T;
